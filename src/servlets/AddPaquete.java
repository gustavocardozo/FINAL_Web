	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			if (request.getSession().getAttribute("usuario") != null) {
				...
				String where = "WHERE DESDE=" + request.getParameter("origen") + " AND HACIA= "
			...
			if (request.getSession().getAttribute("usuario") != null) {
				if ((boolean) request.getSession().getAttribute("doGet")) {
					...
					if (request.getParameter("idVuelo").equals("") || request.getParameter("idVuelo").equals("0")) {
						...
						Integer idPaquete = Integer.parseInt((request.getParameter("idPaquete").equals("")) ? "0"
								: request.getParameter("idPaquete"));
